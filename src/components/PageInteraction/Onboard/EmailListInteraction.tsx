      ...
      <div className='flex flex-row gap-6 max-h-[575px] relative'>
        <div className='flex flex-col gap-3'>
          ...
          <div className='flex flex-col max-h-full overflow-auto gap-2 noScrollbar'>
            {incomingSenders.senders.map(({ from, subject, mailId }) => {
              return (
                <EmailSenderButton
                  ...
                  tag={'category'}
                  type='subscribe'
                  isActivated={selectedEmailList.includes(from.address)}
                  setIsActivated={isActivated =>
                    isActivated ? handleAddEmail(from.address) : handleRemoveEmail(from.address)
                  }
                />
              );
            })}
